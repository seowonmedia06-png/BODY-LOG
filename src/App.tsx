...
export default function App() {
  ...
  const handleSaveScannedRecord = (data: {
    ...
    const rawDateStr = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}T${pad(now.getHours())}:${pad(now.getMinutes())}`;
    const dateLabelStr = `${pad(now.getMonth() + 1)}.${pad(now.getDate())}`;
